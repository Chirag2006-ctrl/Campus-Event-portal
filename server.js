...
export function createApp({ dbPath = process.env.DB_PATH || resolve(root, 'data/campus.db'), adminToken = process.env.ADMIN_TOKEN || '', rateLimit = 30 } = {}) {
  ...
  const server = http.createServer(async (req,res) => {
    ...
    try {
      ...
      if(req.method === 'GET' && url.pathname === '/api/health') { db.prepare('SELECT 1').get(); return json(res,200,{status:'ok',database:'connected',storage:'SQLite'}); }
      if(req.method === 'GET' && url.pathname === '/api/events') {
        const events=db.prepare(`SELECT e.*, (SELECT count(*) FROM registrations r WHERE r.event_id=e.id) AS registered FROM events e ORDER BY starts_at`).all();
        return json(res,200,{events:events.map(e=>({...e,remaining:e.capacity-e.registered}))});
      }
      if(req.method === 'GET' && url.pathname === '/api/admin/registrations') {
        ...
        return json(res,200,{registrations:db.prepare('SELECT r.id,r.name,r.email,r.created_at,e.title AS event FROM registrations r JOIN events e ON e.id=r.event_id ORDER BY r.created_at DESC').all()});
      }
      if(req.method === 'POST' && url.pathname === '/api/registrations') {
        ...
        const event=db.prepare('SELECT * FROM events WHERE id=?').get(input.eventId);
